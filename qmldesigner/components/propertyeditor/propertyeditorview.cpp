...
QString getSpecificsDir()
{
#if (defined Q_OS_WIN) || (defined Q_OS_MAC)
    qDebug("----- getSpecificsDir begin -----");
...
#   if (defined Q_OS_MAC)
        ...
        if(!dir.cdUp()) { break; }
        if(!dir.cd("Resources")) { break; }

#   elif (defined Q_OS_WIN)

        if(!dir.cdUp()) { break; }
        if(!dir.cd("share")) { break; }
        if(!dir.cd("qtcreator")) { break; }

#   endif

        if(!dir.cd("qmldesigner")) { break; }
        if(!dir.cd("customComponents")) { break; }
        if(!dir.cd("PropertyTemplates")) { break; }

        qDebug() << "Current specifics path:" << dir.path();
        qDebug("----- getSpecificsDir end -----");
...
void getAvailableSpecifics(const QString &specificsSourceDir, const QString &currentDir, QHash< QString, QUrl > *target)
{
    for(const auto &now: QDir(currentDir).entryInfoList(QDir::Files))
    {
        auto fileName = now.filePath();
        fileName.replace(specificsSourceDir + "/", "");
        if(fileName.endsWith("Specifics.qml"))
        {
            fileName.replace("Specifics.qml", "");
            fileName.replace("/", ".");
            (*target)[fileName] = QUrl(now.filePath());
        }
    }

    for(const auto &now: QDir(currentDir).entryInfoList(QDir::AllDirs | QDir::NoDotAndDotDot))
    {
        getAvailableSpecifics(specificsSourceDir, now.filePath(), target);
    }
}
...
void PropertyEditorView::setupQmlBackend()
{
    ...
    if (m_selectedNode.isValid()) {
        ...
        foreach (const NodeMetaInfo &metaInfo, hierarchy) {
            static QHash< QString, QUrl > *g_availableSpecifics = NULL;
            if(!g_availableSpecifics)
            {
                g_availableSpecifics = new QHash< QString, QUrl >;
                getAvailableSpecifics( getSpecificsDir(), getSpecificsDir(), g_availableSpecifics );
            }

            auto typeName = metaInfo.typeName();
            if(typeName.startsWith("qrc:/"))
            {
                typeName.replace("qrc:/", "");
                typeName.replace("/", ".");
                typeName.replace("..", ".");
            }

            if(g_availableSpecifics->contains(typeName))
            {
                qmlSpecificsFile = "file:///" + g_availableSpecifics->value(typeName).toString();
                diffClassName = typeName;
                break;
            }

            if (PropertyEditorQmlBackend::checkIfUrlExists(qmlSpecificsFile))
                break;
            qmlSpecificsFile = PropertyEditorQmlBackend::getQmlFileUrl(typeName + QStringLiteral("Specifics"), metaInfo);
            diffClassName = typeName;
        }
    }
